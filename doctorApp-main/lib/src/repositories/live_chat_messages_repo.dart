...
getLiveChatMessagesRepo(
  ...
  if (responseCheck) {
    ...
    if (Get.find<LiveChatController>().getLiveChatMessagesModel.success ==
      ...
      jsonEncode(Get.find<LiveChatController>().messageList);
      log("${jsonEncode(Get.find<LiveChatController>().getLiveChatMessagesModel)} Get Live Chat Messages");
      log("${Get.find<LiveChatController>().messageList.length} MESSAGELIST");
      log("${Get.find<LiveChatController>().messageList} 777");
    } else {
      Get.find<LiveChatController>().updateGetMessagesLoader(false);
    }
  } else {
...
getServiceChatMessagesRepo(
  ...
  if (responseCheck) {
    ...
    if (Get.find<LiveChatController>()
      ...
      log("${jsonEncode(Get.find<LiveChatController>().getLiveServiceChatMessagesModel)} Get Live Service Chat Messages");
      log("${Get.find<LiveChatController>().serviceMessageList.length} MESSAGELIST");
      log("${Get.find<LiveChatController>().serviceMessageList} 777");
    } else {
      Get.find<LiveChatController>().updateGetServiceMessagesLoader(false);
    }
  } else {
